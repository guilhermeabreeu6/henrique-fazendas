...
export default function Diferenciais() {
  return (
    <section id="diferenciais" className="scroll-mt-20 bg-[#F5F1E8] py-24 lg:py-28">
        ...
        <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-4 lg:gap-8">
          {ITEMS.map((item, index) => (
            <Reveal key={item.title} delay={index * 90}>
              <div className="h-full rounded-2xl border border-[#1F3D2B]/10 bg-white/60 p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
